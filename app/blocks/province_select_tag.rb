class ProvinceSelectTag < Liquid::Tag
  include ERB::Util
  ...
  def initialize(tag_name, markup, tokens)
    if markup =~ Syntax
      ...
      raise SyntaxError.new("Syntax Error in 'province_select' - Valid syntax: province_select [name]")
    end
  ...
  def render(context)
    ...
    site_country_id = nil
    if attributes['site_country_id']
      site_country_id = parse_attribute(attributes.delete('site_country_id'))
    end
    ...
    conditions = []
    if site_country_id
      conditions = ['site_country_id = ?', site_country_id]
    end
    SiteProvince.active.find(:all, :conditions => conditions).each do |site_province|
      selected = " selected=\"selected\"" if found_in_model && site_province.id.to_s == found_value.to_s
      rvalue += "<option value=\"#{site_province.id}\"#{selected}>#{html_escape(site_province.province.name)}</option>"
    end
...
Liquid::Template.register_tag('province_select', ProvinceSelectTag)